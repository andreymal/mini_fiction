 ...
 *
 * Hypher is licensed under the three clause BSD license.
 ...
 */
Hypher.prototype.hyphenateDOM = function(nodes) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        ...
        if (node.nodeType === document.TEXT_NODE) {
            node.nodeValue = this.hyphenateText(node.nodeValue);
        } else if (node.childNodes && node.childNodes.length > 0) {
            this.hyphenateDOM(Array.prototype.slice.call(node.childNodes));
        }
    }
};
